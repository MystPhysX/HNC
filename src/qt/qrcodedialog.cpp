...
{
    QString ret = QString("helleniccoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non HNC unit input in lnReqAmount, we generate the URI with HNC as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::HNC, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
